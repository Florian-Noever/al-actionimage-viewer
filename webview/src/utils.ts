...
export function makeSearchPredicate(query: string): (name: string) => boolean {
    ...
    if (!q || core.length === 0) { return () => true; }

    if (hasLeading && hasTrailing) { return (name) => normalize(name).includes(core); }
    if (hasTrailing && !hasLeading) { return (name) => normalize(name).startsWith(core); }
    if (hasLeading && !hasTrailing) { return (name) => normalize(name).endsWith(core); }
